defmodule Bee.Schema do
  ...
  defmacro generate_bee(ast) do
    ast = ast[:do] || []
    {_ast, raw_fields} = Macro.postwalk(ast, [], fn
    ...
    quote do
      ...
      def bee_required_fields, do: for {field, opt} <- unquote(raw_fields), opt[:required], do: field
      def bee_not_update_fields, do: for {field, opt} <- unquote(raw_fields), opt[:dont_update], do: field
      def bee_relation_fields, do: for {field, opt} <-  unquote(raw_fields), opt[:relation], do: field
      def bee_embed_fields, do: for {field, opt} <-  unquote(raw_fields), opt[:embed], do: field
      def bee_fields, do: (for {field, _opt} <- unquote(raw_fields), do: field) -- bee_relation_fields()
